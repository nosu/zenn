...
どの値が有効に効いてくるかは、以下のようないくつかの条件で決まってきます。
...
`Default TTL` でレスポンスの TTL を上書きする場合、CDN 側・クライアント側いずれにも影響すますが、`Default TTL` より短い `Client TTL` を明示的に設定した場合には、`Client TTL` によってクライアント側でのキャッシュ期間を CDN 側のキャッシュ期間と別に設定できます。

（一方で、`FORCE_CACHE_ALL` において、`Client TTL` >= `Default TTL` の場合は `Client TTL` の値は意味がなく、クライアントがローカルで持つキャッシュの有効期限と、CDN 側で持つキャッシュの有効期限がどちらも `Default TTL` の秒数に強制的に上書き設定されます。）
